...
async function cargarReporteMorosidad() {
  ...
  try {
    ...
    morosidadData = data.sociosMorosos || data.morosos || [];

    document.getElementById('kpiTotalMorosos').textContent = data.totalMorosos ?? morosidadData.length;
    document.getElementById('kpiMontoMora').textContent = `$${Number(data.deudaTotalAcumulada || 0).toFixed(2)}`;

    const casosCorte = morosidadData.filter((s) => (s.mesesAdeudados ?? s.mesesAtrasados ?? 0) >= 3).length;
...
function renderTablaMorosidad(socios) {
  ...
  const filtrados = socios.filter((s) => (s.mesesAdeudados ?? s.mesesAtrasados ?? 0) >= minMeses);
  ...
  tbody.innerHTML = filtrados
    .map((s) => {
      ...
      const nombreSocio = s.nombresCompletos || `${s.apellidos || ''} ${s.nombres || ''}`.trim() || 'Socio';
      const montoDeuda = s.deudaTotalPendiente ?? s.totalAdeudado ?? 0;
      const fechaAntigua = s.fechaDeudaMasAntigua || s.fechaCorteInicial;
      ...
      return `
        <tr>
          ...
          <td style="font-size: 0.8rem; color: #64748b;">${fechaAntigua ? fechaAntigua.split('T')[0] : 'Período actual'}</td>
          <td style="text-align: right; font-weight: 800; color: #dc2626; font-size: 1rem;">
            $${Number(montoDeuda).toFixed(2)}
          </td>
...
async function cargarReporteSectores() {
  ...
  try {
    ...
    sectoresData = res.data || res.sectores || (Array.isArray(res) ? res : []);
...
function renderTablaSectores(sectores) {
  ...
  tbody.innerHTML = sectores
    .map((sec) => {
      const facturado = sec.totalFacturado ?? (Number(sec.totalCobrado || sec.totalRecaudado || 0) + Number(sec.totalEnMora || sec.totalPendienteMora || 0));
      const cobrado = sec.totalCobrado ?? sec.totalRecaudado ?? 0;
      const mora = sec.totalEnMora ?? sec.totalPendienteMora ?? 0;
      ...
      return `
        <tr>
          ...
          <td style="text-align: right; font-weight: 700;">$${Number(facturado).toFixed(2)}</td>
          <td style="text-align: right; font-weight: 800; color: #16a34a;">$${Number(cobrado).toFixed(2)}</td>
          <td style="text-align: right; font-weight: 800; color: #dc2626;">$${Number(mora).toFixed(2)}</td>
        </tr>
      `;
    })
    .join('');
